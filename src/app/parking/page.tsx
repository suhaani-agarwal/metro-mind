...
export default function TrainParkingPage() {
  ...
  const fetchTrains = async () => {
    try {
      const response = await axios.get("http://localhost:8000/api/nightly/trains");
      // Convert all train IDs to TM format for display
      const tmTrains = response.data.trains.map((train: string) => mapToTMFormat(train));
      setTrains(tmTrains);
    } catch (error) {
  ...
  const fetchParkingData = async () => {
    try {
      const response = await axios.get("http://localhost:8000/api/nightly/parking/assignments");
      // Convert all train IDs in parking data to TM format
      const tmParkingData = response.data.assignments?.map((assignment: TrainParking) => ({
        ...assignment,
  ...
  /** Occupancy check */
  const isPositionOccupied = (track: string, position: number, trackType: "parking" | "maintenance") => {
  ...
  const getTrainAtPosition = (track: string, position: number, trackType: "parking" | "maintenance") => {
    const bay = getBayFromTrack(track, trackType);
  ...
  const getBayFromTrack = (track: string, trackType: "parking" | "maintenance") => {
  ...
  const handleAssignParking = async () => {
    ...
    if (isPositionOccupied(selectedTrack, selectedPosition, trackType) && !isEditing) {
      alert(`Position ${getBayFromTrack(selectedTrack, trackType)} is already occupied!`);
      return;
    }
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(`http://localhost:8000/api/nightly/parking/assignment/${editingTrain}`, payload);
        alert(`Parking assignment updated for Train ${selectedTrain} ✅`);
      } else {
        await axios.post("http://localhost:8000/api/nightly/parking/assignment", payload);
        alert(`Train ${selectedTrain} assigned to ${getPositionDisplayName(bay, selectedPosition)} ✅`);
      }
  ...
  const handleRemove = async (trainId: string) => {
    if (confirm(`Remove parking assignment for Train ${trainId}?`)) {
      try {
        await axios.delete(`http://localhost:8000/api/nightly/parking/assignment/${trainId}`);
  ...
  const handleMarkDeparted = async (trainId: string) => {
    try {
      const assignment = parkingData.find((a) => a.train_id === trainId && !a.departure_time);
      if (assignment) {
        await axios.put(`http://localhost:8000/api/nightly/parking/assignment/${trainId}`, {
          ...assignment,
  ...
  const parkingTracksList = parkingTracks.filter((track) => track.type === "parking");
  const maintenanceTracks = parkingTracks.filter((track) => track.type === "maintenance");
    ...
    >
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        >
          ...
          <p className="text-slate-300">Manage parking assignments for 12 stabling tracks and 5 maintenance bays</p>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-semibold text-slate-50">
                {isEditing ? "Edit Parking Assignment" : "Assign Parking Position"}
              </h2>
            </div>
            
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-300">Select Train</label>
                <select
                ...
                >
                  <option value="" className="bg-slate-800">Select a train</option>
                  {trains.map((train) => (
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-300">Assignment Type</label>
                <select
                  className="w-full p-4 rounded-xl border border-slate-600/50 text-slate-50 transition-all duration-300 hover:border-emerald-400/50 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20 outline-none backdrop-blur-sm"
                  style={{ backgroundColor: "rgba(30, 41, 59, 0.6)" }}
                  value={status}
                  onChange={(e) => handleStatusChange(e.target.value as "parking" | "maintenance")}
                >
                  <option value="parking" className="bg-slate-800">Parking</option>
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-300">
                  {status === "maintenance" ? "Maintenance Bay Selection" : "Stabling Track Selection"}
                </label>
                <select
                  className="w-full p-4 rounded-xl border border-slate-600/50 text-slate-50 transition-all duration-300 hover:border-emerald-400/50 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20 outline-none backdrop-blur-sm"
                  style={{ backgroundColor: "rgba(30, 41, 59, 0.6)" }}
                ...
                >
                  ...
                  {(status === "maintenance" ? maintenanceTracks : parkingTracksList)
                    .map((track) => (
                      <option key={track.track} value={track.track} className="bg-slate-800">
                        {status === "maintenance" ? track.track : `Track ${track.track}`}
                      </option>
                    ))
                  }
                </select>
              ...
              {selectedTrack && status === "parking" && (
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-slate-300">Position Selection</label>
                  <select
                    className="w-full p-4 rounded-xl border border-slate-600/50 text-slate-50 transition-all duration-300 hover:border-emerald-400/50 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20 outline-none backdrop-blur-sm"
                    style={{ backgroundColor: "rgba(30, 41, 59, 0.6)" }}
                  ...
                  >
                    <option value={0} className="bg-slate-800">Select a position</option>
                    {getPositionsForSelectedTrack().map((position) => {
                      const isOccupied = isPositionOccupied(selectedTrack, position, "parking");
                      const occupyingTrain = getTrainAtPosition(selectedTrack, position, "parking");
                      ...
                      return (
                        ...
                        >
                          Position {position} 
                          {isOccupied ? ` (Occupied by Train ${occupyingTrain?.train_id})` : " (Available)"}
                        </option>
                      );
                    })}
                  </select>
                    ...
                    {selectedPosition === 1 && (
                      <div className="text-sm text-slate-400 mt-1">
                        {isPositionOccupied(selectedTrack, 1, "maintenance") 
                          ? `Occupied by Train ${getTrainAtPosition(selectedTrack, 1, "maintenance")?.train_id}`
                          : "Available"
                        }
                      </div>
                    )}
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-300">Assignment Notes</label>
                <textarea
              ...
              <button
                ...
                style={{
                  background: !selectedTrain || !selectedTrack || !selectedPosition
                    ? "rgba(100, 116, 139, 0.5)"
                    : isEditing 
                      ? "linear-gradient(135deg, #fbbf24 0%, #f59e0b 100%)"
                      : "linear-gradient(135deg, #38bdf8 0%, #06d6a0 100%)",
                  boxShadow: !selectedTrain || !selectedTrack || !selectedPosition
                    ? "none"
                    : isEditing
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-semibold text-slate-50">Real-Time Parking Layout</h2>
            </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {parkingTracksList.map((track) => (
                    <div 
                      ...
                      style={{ backgroundColor: "rgba(30, 41, 59, 0.6)" }}
                    >
                      <h4 className="font-semibold text-slate-50 text-center mb-3">Track {track.track}</h4>
                      <div className="space-y-2">
                        {track.positions.map((position) => {
                          const assignment = getTrainAtPosition(track.track, position, "parking");
                          return (
                            <div
                              key={position}
                            ...
                            >
                              ...
                              {assignment ? (
                                <div className="text-sm mt-1">
                                  <div className="font-semibold">Train {assignment.train_id}</div>
                                </div>
                              ) : (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  {maintenanceTracks.map((track) => (
                    <div 
                      ...
                      style={{ backgroundColor: "rgba(30, 41, 59, 0.6)" }}
                    >
                      <h4 className="font-semibold text-slate-50 text-center mb-3">{track.track}</h4>
                      {track.positions.map((position) => {
                        const assignment = getTrainAtPosition(track.track, position, "maintenance");
                        return (
                          <div
                            key={position}
                          ...
                          >
                            {assignment ? (
                              <div className="mt-1">
                                <div className="font-semibold text-sm">Train {assignment.train_id}</div>
                              </div>
                            ) : (
                              <div className="text-sm mt-1">Available</div>
                            )}
        ...
        >
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-semibold text-slate-50">Active Parking Assignments</h2>
          </div>
          
          {parkingData.filter(a => !a.departure_time).length === 0 ? (
            <div className="text-center py-12">
              <div className="text-6xl mb-4">🚆</div>
              <p className="text-slate-400 text-lg">No active parking assignments.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              ...
              >
                <table className="min-w-full">
                  ...
                  >
                    <tr>
                      <th className="px-6 py-4 text-left text-sm font-semibold text-slate-300">Train ID (TM)</th>
                  ...
                  <tbody className="divide-y divide-slate-600/30">
                    {parkingData
                      ...
                      .map((assignment) => (
                      <tr key={assignment.train_id} className="hover:bg-slate-700/30 transition-colors duration-200">
                        <td className="px-6 py-4 font-medium text-slate-50">Train {assignment.train_id}</td>
                        <td className="px-6 py-4 text-slate-300">
                          {getPositionDisplayName(assignment.bay, assignment.position)}
                        </td>
                        ...
                        <td className="px-6 py-4 text-slate-400 max-w-xs truncate text-sm">{assignment.notes}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
